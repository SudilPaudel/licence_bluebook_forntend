...
const ElectricNewBluebook = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Vehicle Model in submitData:', submitData.vehicleModel);
      console.log('Vehicle Model type:', typeof submitData.vehicleModel);
      const response = await fetch(`${import.meta.env.VITE_API_URL}/electric-bluebook`, {
        method: 'POST',
        headers: {
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="transition-all duration-200 hover:scale-[1.02]">
                  ...
                  <input
                    type="number"
                    name="vehicleBatteryCapacity"
                    value={formData.vehicleBatteryCapacity}
                    onChange={handleChange}
                    className={`mt-1 block w-full border rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-nepal-blue transition-all duration-200 ${
                      errors.vehicleBatteryCapacity ? 'border-red-400' : 'border-gray-200'
                    }`}
...
export default ElectricNewBluebook